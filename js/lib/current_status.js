...
function draw_latest_publications(publications, JIF_threshold){
	...
	for (item in published_date_list){
		if (issns_over_10.indexOf(published_date_list[item][1]["journal"]["issn"]) > 0){
			// console.log(published_date_list[item][1]);
	...
	for (pub in show_list){
		...
		$("#latest_"+pub).append('<div class="latest_publication_title"><a target="_blank" href="">'+show_list[pub]["title"]+'</a></div>');
		...
		$("#latest_"+pub).append('<div class="latest_publication_authors"><p>'+author_string+'</p></div>');
	}
}
